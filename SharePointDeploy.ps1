...
function InstallSolution
{
	...
	elseif (($Solution) -and ($Solution.Added -eq $true))
	{
		if ($Solution.ContainsWebApplicationResource)
		{
			if($Force -eq $true)
			{			
				Install-SPSolution -Identity $WspName -GacDeployment -CompatibilityLevel $CompatibilityLevel -WebApplication $WebAppUrl -Force
			}
			else
			{
		...
		else
		{
			if($Force -eq $true)
			{			
				Install-SPSolution -Identity $WspName -GacDeployment -CompatibilityLevel $CompatibilityLevel -Force
			}
			else
			{
...
$SiteUrl = $WebApplicationUrl + "/sites/mysite/"
$WspPath = "E:\Dev\Build\"
$WspName = "MySolution.wsp"
$WspLocation = $WspPath + $WspName

$FeatureId = "ffe5be0c-86ea-4138-be90-2878c44df9fb"

cls
Write-Host "Script started at: $(Get-Date -Format g)" -foreground "red"

#Disable Features
disablefeature -webapplicationurl $webapplicationurl -siteurl $siteurl -featureguid $FeatureId

UninstallSolution -WspName $WspName
AddSolution -WspLocation $WspLocation
InstallSolution -WspName $WspName -WebAppUrl $WebApplicationUrl

#Enable Features
EnableFeature -WebApplicationUrl $WebApplicationUrl -SiteUrl $SiteUrl -FeatureGuid $FeatureId -force $True
